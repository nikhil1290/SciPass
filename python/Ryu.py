...
class Ryu(app_manager.RyuApp):
    ...
    def changeSwitchForwardingState(self, dpid=None, domain=None,header=None, actions=None, command=None, idle_timeout=None, hard_timeout=None, priority=1):
        ...
        if(ofp.OFP_VERSION == ofproto_v1_3.OFP_VERSION):
            ...
            flow = self.OF13_flow(datapath, domain=domain, header=header, actions=actions, command=command, idle_timeout=idle_timeout, hard_timeout=hard_timeout, priority=priority)
        elif (ofp.OFP_VERSION == ofproto_v1_0.OFP_VERSION):
            self.logger.debug("Generating OF 1.0 FlowMod")
            flow = self.OF10_flow(dp=datapath, domain=domain, header=header, actions=actions, command=command, idle_timeout=idle_timeout, hard_timeout=hard_timeout, priority=priority)
        else:
    ...
    def process_flow_removed_of10(self, flow, dp):
        ...
        obj = { 'nw_src': str(src_prefix), 'nw_dst' : str(dst_prefix)}
